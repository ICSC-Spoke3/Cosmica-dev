...
__global__ void histogram_atomic(const float* in, const float LogBin0_lowEdge, const float DeltaLogT, const int Nbin, const unsigned long Npart, float *out, int *Nfailed ) {
    ...
    if (id<Npart){ 
        if (log10(in[id])>LogBin0_lowEdge){
            ...
            atomicAdd(&out[blockIdx.x*Nbin+DestBin], 1);
            // exp(alphapath[id])
        }
