...
__global__ void HeliosphericProp(int Npart_PerKernel, float Min_dt, float Max_dt, float TimeOut, struct QuasiParticle_t QuasiParts_out, int* PeriodIndexes, struct PartDescription_t pt, curandStatePhilox4_32_10_t* CudaState, float* RMaxs) {
  ...
  if (id<Npart_PerKernel) {
    ...
    while(smem[threadIdx.x + 5*blockDim.x]>=0 && smem[threadIdx.x + 4*blockDim.x]<=TimeOut){
      ...
      float dt = Max_dt;
      ...
      #else
        // Evaluate the convective-diffusive tensor and its decomposition
        KSym = DiffusionTensor_symmetric(PeriodIndexes[id], smem[threadIdx.x + 5*blockDim.x], smem[threadIdx.x], smem[threadIdx.x + blockDim.x], smem[threadIdx.x + 2*blockDim.x], smem[threadIdx.x + 3*blockDim.x], pt, RandNum.w);

        int res = 0;
        Ddif = SquareRoot_DiffusionTerm(smem[threadIdx.x + 5*blockDim.x], KSym, smem[threadIdx.x], smem[threadIdx.x + blockDim.x], &res);
        ...
        // Evaluate advective-drift vector
        AdvTerm = AdvectiveTerm(PeriodIndexes[id], smem[threadIdx.x + 5*blockDim.x], KSym, smem[threadIdx.x], smem[threadIdx.x + blockDim.x], smem[threadIdx.x + 2*blockDim.x], smem[threadIdx.x + 3*blockDim.x], pt);

        // Evaluate the energy loss term
        en_loss = EnergyLoss(PeriodIndexes[id], smem[threadIdx.x + 5*blockDim.x], smem[threadIdx.x], smem[threadIdx.x + blockDim.x], smem[threadIdx.x + 2*blockDim.x], smem[threadIdx.x + 3*blockDim.x], pt.T0);
