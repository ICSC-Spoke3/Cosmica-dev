...
from files_utils import load_simulation_outputs_yaml, load_simulation_output, load_simulation_outputs
from physics_utils import evaluate_modulation
...
def evaluate_output(output, experimental_data, lis, rig_in=False, plot_path=None):
    """
    ...
    """
    if len(output) == 1:
        outputs = load_simulation_outputs_yaml(output)
    else:
        outputs = load_simulation_outputs(output)

    sim_en_rig, sim_j_mod, j_lis = evaluate_modulation(outputs, lis, rig_in=True, rig_out=True)
