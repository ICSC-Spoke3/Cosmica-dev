...
from isotopes import find_ion_or_isotope
from physics_utils import rig_to_en
from files_utils import load_simulation_list, load_heliospheric_parameters, load_lis, load_experimental_data
...
def create_input_file(k0vals, h_par, exp_data, input_dir, sim_el, tot_npart_per_bin=1200,
    ...
    for k0val in k0vals:
        ...
        for ion in ions:
            ...
            with open(f'{yml['output_path']}.yaml', 'w') as f:
...
def make_input_from_sim(ROOTDIR, input_dir, sim_el, k0_array):
    ...
    exp_data = load_experimental_data(pexp, sim_el[2], (3, 11))

    sims_dict = create_input_file(
        k0_array, h_par, exp_data, input_dir, sim_el, force_execute=True
    )
...
if __name__ == "__main__":
    ...
    files = make_input_from_sim(ROOTDIR, dirname(__file__), sim_el, [0.])
    print(files)