...
__device__ float3
Diffusion_Tensor_In_HMF_Frame(unsigned char, signed char, float, float, float, float, float, float3 &);

__device__ float Diffusion_Coeff_heliosheat(unsigned char, float, float, float, float, float, float &);
#endif
