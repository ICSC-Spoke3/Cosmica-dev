...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    #pragma omp parallel
    {
        ...
        struct LaunchParam_t prop_launch_param = RoundNpart(NParts, device_prop, VERBOSE, SetWarpPerBlock);
        ...
        for (int iR=gpu_id; iR<NInitRig ; iR+=NGPUs) {
            ...
            float DeltaLogR= log10(1.+RelativeBinAmplitude);
            ...
            printf("---\nResults[%d].BoundaryDistribution =\n", iR);
            for (int i=0; i<Results[iR].Nbins; i++) printf("%f\t", Results[iR].BoundaryDistribution[i]);
        }
