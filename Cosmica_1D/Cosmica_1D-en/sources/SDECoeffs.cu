...
__device__ struct Tensor3D_t trivial_SquareRoot_DiffusionTensor(float K0, float T, PartDescription_t pt){ // int InitZone, int HZone, float T, struct PartDescription_t pt
  ...
  // if (HZone<Heliosphere.Nregions) Ddif.rr = K0*beta_(T, pt.T0)*Rigidity(T, pt);

  // else Ddif.rr = HS[HZone].k0*beta_(T, pt.T0)*Rigidity(T, pt);

  Ddif.rr = K0*beta_(T, pt.T0)*Rigidity(T, pt);
...
__device__ float trivial_EnergyLoss(float T, float r, float V0, struct PartDescription_t pt){ // int InitZone, int HZone, float T, float r, struct PartDescription_t pt
  ...
  return 2.*V0*(T+2.*pt.T0)/(T+pt.T0)*T/(3.*r);
}

__device__ float trivial_LossTerm(float T, float r, float V0, struct PartDescription_t pt){ // int InitZone, int HZone, float T, float r, struct PartDescription_t pt

  // float V0=(HZone<Heliosphere.Nregions)?LIM[HZone+InitZone].V0:HS[InitZone].V0;

  // if (HZone<Heliosphere.Nregions) return 2.*V0/r *( 1./3.*( T*T+2.*T*pt.T0+ 2.*pt.T0*pt.T0 )/( (T+pt.T0)*(T+pt.T0) ) -1. );

  // else return 0.;

  return 2.*V0/r *( 1./3.*( T*T+2.*T*pt.T0+ 2.*pt.T0*pt.T0 )/( (T+pt.T0)*(T+pt.T0) ) -1. );
}
