...
/////////////////////////////////////////////////////////////////////////////
void RescaleToEffectiveHeliosphere(struct HeliosphereBoundRadius_t &Rbound, struct vect3D_t &part)
{
...
float K0Fit_ssn(int p, int SolarPhase, float ssn, float *GaussVar){
  ...
  float k0;
  if(p>0.){
    if(SolarPhase==0)/*Rising*/   {k0=0.0002743-2.11e-6*ssn+1.486e-8*ssn*ssn-3.863e-11*ssn*ssn*ssn;   *GaussVar=0.1122;}
    else             /*Declining*/{k0=0.0002787-1.66e-6*ssn+4.658e-9*ssn*ssn-6.673e-12*ssn*ssn*ssn;   *GaussVar=0.1324;}
  }else{
...
float g_low(int SolarPhase, int Polarity, float tilt){
...
float rconst(int SolarPhase, int Polarity, float tilt){
  /*Authors: 2022 Stefano - update 2023 GLV*/
...
__device__ float3 Diffusion_Tensor_In_HMF_Frame(unsigned char InitZone,signed char HZone, float r, float theta, float beta, float P, float GaussRndNumber, float3 &dK_dr) {
  /*Authors: 2022 Stefano */
  ...
  */
  ...
  float k0_paral = LIM[HZone+InitZone].k0_paral[high_activity];
  float k0_perp = LIM[HZone+InitZone].k0_perp[high_activity];
  float GaussVar = LIM[HZone+InitZone].GaussVar[high_activity];
  float g_low = LIM[HZone+InitZone].g_low;
  float rconst = LIM[HZone+InitZone].rconst;


  // Kpar = k0 * beta/3 * (P/1GV + glow)*( Rconst+r/1AU) with k0 gaussian distributed
  Ktensor.x = (k0_paral + GaussRndNumber*GaussVar*k0_paral ) ;
  dK_dr.x   = Ktensor.x;
  Ktensor.x *= beta/3. * (P+g_low) * (rconst+r);
  dK_dr.x   *= beta/3. * (P+g_low) ;

#ifndef rho_1
  #define rho_1 0.065 // Kpar/Kperp (ex Kp0)
#endif
  // Kperp1 = rho_1(theta)* k0 * beta/3 * (P/1GV + glow)*( Rconst+r/1AU)
  Ktensor.y = rho_1 * k0_perp * beta/3. * (P+g_low) * (rconst+r);
  dK_dr.y   = rho_1 * k0_perp * beta/3. * (P+g_low) ;
#ifndef PolarEnhanc
  ...
  Ktensor.z = rho_1 * k0_perp * beta/3. * (P+g_low) * (rconst+r);
  dK_dr.z   = rho_1 * k0_perp * beta/3. * (P+g_low) ;
  return Ktensor;
}

__device__ float Diffusion_Coeff_heliosheat(signed char HZone, float r, float th, float phi, float beta, float P, float &dK_dr) {
  /*Authors: 2022 Stefano */
  /* * description: evaluate the diffusion tensor in the HMF frame, i.e. Kparallel & Kperpendicular.
      \param HZone   Zone in the Heliosphere
      \param r      solar distance
  ...
  */
  ...
  float RhpDirection=Boundary(th, phi, Heliosphere.RadBoundary_effe[HZone].Rhp_nose, Heliosphere.RadBoundary_effe[HZone].Rhp_tail);
#ifndef HPB_SupK
  ...
  if ((r>RhpDirection-5))
                        {
      return HS[HZone].k0*beta*(P)*SmoothTransition(1, 1./HPB_SupK, RhpDirection-(HP_width/2.), HP_SupSmooth,r);
                        }
  else {
