...
__device__ vect3D_t Drift_PM89(unsigned char InitZone, signed char HZone, float r, float th, float phi, float R, struct PartDescription_t pt){
  ...
  float Ka   = (sign(pt.Z)*(GeV)*beta_R(R,pt))*R/3;  /* sign(Z)*beta*P/3 constant part of antisymmetric diffusion coefficient */
  ...
  float dthetans = fabs((GeV/(c*aum))*(2.*r*R)/((Asun)*sqrt( 1+Gamma_Bfield(r,TiltPos_th,Vsw)*Gamma_Bfield(r,TiltPos_th,Vsw)+((IsPolarRegion)?delta_Bfield(r,TiltPos_th)*delta_Bfield(r,TiltPos_th):0))));  /*dTheta_ns = 2*R_larmor/r*/
  ...
  if (IsPolarRegion){
    ...
    float HighRigiSupp=LIM[HZone+InitZone].plateau+(1.-LIM[HZone+InitZone].plateau)/(1.+exp(HighRigiSupp_smoothness*(R-HighRigiSupp_TransPoint)));
