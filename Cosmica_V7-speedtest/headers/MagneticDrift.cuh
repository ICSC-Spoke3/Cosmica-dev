...
__device__ float eval_Ka(PartDescription_t, float);
__device__ float eval_fth(float, float);
__device__ float eval_Dftheta_dtheta(float, float);
__device__ float eval_HighRigiSupp(int, int, float);
__device__ float eval_E_drift(bool, float, float, float);
__device__ float eval_C_drift_reg(float, float, float, float, float, int, int, float, float, float);
__device__ float eval_C_drift_ns(float, float, float, float, float, int, int, float,float, float, float);
__device__ vect3D_t Drift_PM89(unsigned char, signed char, float, float, float, float, bool, float, float, float, float, float, float, float);
