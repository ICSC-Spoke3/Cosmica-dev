...
def convert_txt_to_yaml(txt, partname):
    ...
    return yml
    # return yml, yaml.dump(yml, sort_keys=False, width=float("inf"))
...
def find_matching_ion(txt, ions):
    for v in txt.split('_'):
        if v.lower() in ions:
            return v.lower()
    return None
...
def convert_multi_txt_to_yaml(txts, yml, mod =None):
    ...
    for txt in txts:
        ...
        with open(txt, 'r') as f:
            txt = f.read()
        tmp = convert_txt_to_yaml(txt, ion)
        if data is None:
            data = tmp
        else:
...
def find_matching_file(ion, txts):
    for t in txts:
        if ion in t.lower():
            return t
    return None

def convert_yaml_to_multi_txt(yml, txts):
    with open(yml, 'r') as f:
        yml = yaml.load(yml, Loader=yaml.SafeLoader)
    for ion in yml['isotopes'].keys():
        txt = find_matching_file(ion, txts)
        with open(txt, 'w') as f:
            f.write(convert_yaml_to_txt(yml, ion))
...
if __name__ == "__main__":
    ...
    if sys.argv[1].endswith('.txt'):
        ...
        convert_multi_txt_to_yaml(input_files, output_file)
    else:
        input_file = sys.argv[1]
        output_files = sys.argv[2:]
        convert_yaml_to_multi_txt(input_file, output_files)
