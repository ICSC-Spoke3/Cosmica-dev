...
class PSO_new(object):
	...
	def spawn_child(self):
		if self.spawned_solution_counter > self.numberofparticles:
			raise Exception("Spawned more solution than selected solution")
		else:
			solution = self.Solutions[self.spawned_solution_counter]
			self.spawned_solution_counter+=1
			return solution
	...
	def ask(self, verbose=False):
		...
		if self._ask_number >= len(self.Solutions):
			return None

		solution = self.Solutions[self._ask_number]
		self._ask_number +=1
		return solution

	def tell(self,solution:int|list|tuple|Particle,fitness:int|list = None):
	...
	def InitCreateParticles(self, n, dim, creation_method={ 'name':"uniform"}):
	...
	def _updateSolutionFitness(self,s:Particle,fitness:float=None):
