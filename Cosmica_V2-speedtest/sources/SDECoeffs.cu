...
__device__ struct DiffusionTensor_t DiffusionTensor_symmetric(unsigned char InitZone, signed char HZone, float r, float th, float phi, float Ek, struct PartDescription_t pt, float GaussRndNumber){
  ...
  if (HZone<Heliosphere.Nregions){
    ...
    if (IsPolarRegion){
      // polar region
      KK.rr = Kh.y*sinZeta2 + cosZeta2 * (Kh.x*cosPsi2 + Kh.z*sinPsi2);
      KK.tt = Kh.y*cosZeta2 + sinZeta2 * (Kh.x*cosPsi2 + Kh.z*sinPsi2);
      KK.pp = Kh.x*sinPsi2 + Kh.z*cosPsi2;
      KK.tr = sinZeta*cosZeta*(Kh.x*cosPsi2+Kh.z*sinPsi2-Kh.y);
      ...
      // --> sinZeta = 0 
      // --> cosZeta = +/- 1
      KK.rr =               + cosZeta2 * (Kh.x*cosPsi2 + Kh.z*sinPsi2);
      KK.tt = Kh.y*cosZeta2 ;
      KK.pp = Kh.x*sinPsi2 + Kh.z*cosPsi2;
      KK.tr = 0.;
      KK.pr = -(Kh.x-Kh.z) *sinPsi*cosPsi*cosZeta;
