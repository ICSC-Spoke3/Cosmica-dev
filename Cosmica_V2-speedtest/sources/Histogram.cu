...
__device__ void BlockMax(float* sdata, float* outdata) {
    ...
    // thread index taking into account the shift imposed by the rigidity positions in shared memory array
    unsigned int sdata_id = threadIdx.x + 3*blockDim.x;
...
__global__ void GridMax(const float Nmax, float* indata, float* outdata) {
    // shared memory allocation and filling
...
__global__ void Rhistogram_atomic(const float* R_in, const float LogBin0_lowEdge, const float DeltaLogR , const int Nbin, const unsigned int Npart,  float* R_out) {

    extern __shared__ unsigned int smem[];
    
    int id = threadIdx.x + blockIdx.x * blockDim.x;
    int block_shift = blockIdx.x*Nbin;
    ...
    if (id<Npart) {
        if (log10f(R_in[id])>LogBin0_lowEdge) {
            ...
            int dest_bin = floorf((log10f(R_in[id])-LogBin0_lowEdge)/DeltaLogR);
            atomicAdd(&smem[dest_bin], 1);
        }
    }

    // write partial histogram to global memory
    if (threadIdx.x<Nbin) R_out[threadIdx.x + block_shift] = smem[threadIdx.x];
}

// Unroll the last steps when reduction dimension < warp dimension
__device__ void WarpSum(volatile int* sdata, unsigned int tid) {
...
__global__ void TotalHisto(float* indata, const unsigned int Nbins, const unsigned int Nblocks, float* outdata) {
    // shared memory allocation and filling
    extern __shared__ int shist[];

    unsigned int id = blockIdx.x*blockDim.x + threadIdx.x;
    unsigned int bin_id = 2*threadIdx.x*Nbins + blockIdx.x;

    // First histogram couple merge during shared memory allocation
    // Each block perform one rigidity bin reduction
    if (id<Nbins*Nblocks) shist[threadIdx.x] = indata[bin_id] + indata[bin_id + Nbins];
    ...
    // write result for this block to global mem
    if (threadIdx.x == 0) outdata[blockIdx.x] = (float)shist[0];
}
...
__global__ void histogram_atomic(const float* in, const float* alphapath, const float LogBin0_lowEdge, const float DeltaLogT, const int Nbin, const unsigned long Npart, float *out, int *Nfailed ) {
    ...
    if (id<Npart){ 
        if (log10(in[id])>LogBin0_lowEdge){
            int DestBin = floor( (log10(in[id])-LogBin0_lowEdge)/DeltaLogT ); // evalaute the bin where put event
            atomicAdd(&out[blockIdx.x*Nbin+DestBin], exp(alphapath[id]));
        }
