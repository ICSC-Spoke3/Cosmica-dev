...
__global__ void HeliosphericProp(int Npart_PerKernel, float Min_dt, float Max_dt, float TimeOut, struct QuasiParticle_t QuasiParts_out, int* PeriodIndexes, struct PartDescription_t pt, curandStatePhilox4_32_10_t* CudaState, float* RMaxs, int trivial) {
  ...
  if (id<Npart_PerKernel) {
    ...
    if (trivial) smem[threadIdx.x + 6*blockDim.x] = Zone(smem[threadIdx.x], smem[threadIdx.x + blockDim.x], smem[threadIdx.x + 2*blockDim.x]);
    else         smem[threadIdx.x + 6*blockDim.x] = RadialZone(PeriodIndexes[id], smem[threadIdx.x], smem[threadIdx.x + blockDim.x], smem[threadIdx.x + 2*blockDim.x]);
