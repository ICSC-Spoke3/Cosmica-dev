...
int BestWarpPerBlock (char name[], int verbose) {
    ...
    if      (strstr(name, "NVIDIA A30"))  {BestWarpPerBlock=16;} // cuda cap. 8.0 : 1,2,4,8,16,32 
    else if (strstr(name, "NVIDIA A40"))  {BestWarpPerBlock=16;} // cuda cap. 8.6 : 1,2,3,4,6,8,12,16,24 
...
struct LaunchParam_t RoundNpart (int NPart, cudaDeviceProp GPUprop, bool verbose, int WpB=-1) {
    ...
    launch_param.smem = (int)ceil_int(7*launch_param.Npart*sizeof(float), launch_param.blocks);
