...
__device__ signed char RadialZone(unsigned short InitZone,qvect_t part){
/* Author: SDT - Feb 2022
   ...
   */
	...
	if (part.r<Boundary(part.th, part.phi,rbound.Rhp_nose, rbound.Rhp_tail))
	{   // inside Heliopause boundary
		if (part.r>=Boundary(part.th, part.phi,rbound.Rts_nose, rbound.Rts_tail))
		{   // inside Heliosheat
			return Heliosphere.Nregions;
		}else{
			// inside Termination Shock Boundary
			if (part.r<rbound.Rts_nose) return floor(part.r/rbound.Rts_nose*Heliosphere.Nregions);
