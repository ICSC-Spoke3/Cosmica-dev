...
#define OPTSTR "vi:h"
#define USAGE_MESSAGE "Thanks for using Cosmica, the usage of this program is: "
...
// ------------- load Zones properties ---------
  ...
  for (int izone =0 ; izone<NumberParameterLoaded; izone++){
    ...
    SimParameters.prop_medium[izone].g_low = g_low(IHP[izone].SolarPhase, IHP[izone].SmoothTilt);
    SimParameters.prop_medium[izone].rconst= rconst(IHP[izone].SolarPhase, IHP[izone].SmoothTilt);
